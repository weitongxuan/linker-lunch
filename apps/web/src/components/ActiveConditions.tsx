...
type SetKey = 'cat' | 'excludeCat' | 'cuisine' | 'excludeCuisine';
...
export function ActiveConditions() {
  ...
  const clearAll = () => {
    (['cat', 'excludeCat', 'cuisine', 'excludeCuisine'] as SetKey[]).forEach((k) => dispatch({ type: 'SET_SET_FILTER', key: k, values: [] }));
  ...
  return (
    <div className="conds" aria-label="目前條件">
      ...
      {chips.map((c) => (
        <span key={c.key} className={`chip on${c.text.startsWith('不吃') ? ' exclude' : ''}`}>
          {c.text}
